...
layui.define(['jquery','layer'],function(exports){
  ...
  Class.prototype.init = function(){
    ...
    $.each(options.content,function(index,item){
      spans +='<div class="drag-item" draggable="true" style="display:inline-block"><span><em>'+item+'</em><button type="button" class="close">×</button></span></div>';
    ...
    options.elem.keypress(function(event){  
      ...
      if(keynum == '13'){  
        ...
        if(options.content.indexOf($val) == -1){
          ...
          that.render()
          spans ='<div class="drag-item" draggable="true" style="display:inline-block"><span><em>'+$val+'</em><button type="button" class="close">×</button></span></div>';
          options.elem.before(spans)
        }
